...
export const $filters = atom<{ piece: string, set: string[], status: string, color: string[] }>({ piece: "", set: [], status: "all", color: [] });
...
export async function updateFilters(filters: { piece: string, set: string[], status: string, color: string[] }) {
  $filters.set(filters);
}